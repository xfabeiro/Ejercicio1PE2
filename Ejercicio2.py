...
from ClasesEj2 import EmpleadoFijo, EmpleadoTemporal
...
def Anadir_empleado(diccionario):
    ...
    sueldo = float(
        input("Introduzca el sueldo base mensual del nuevo empleado: "))
    if tipo == "Fijo":
        comp = float(
            input("Introduzca el complemento anual del sueldo del nuevo empleado: "))
        diccionario[nif] = EmpleadoFijo(
            nif, nombre, fechanaz, tipo, fechaalta, sueldo, complemento_anual=comp)
    elif tipo == "Temporal":
        fechabajaaux = input(
        ...
        diccionario[nif] = EmpleadoTemporal(
            nif, nombre, fechanaz, tipo, fechaalta, sueldo, fecha_baja=fechabaja)
...
# Función para mostrar empleados de cumpleaños
